...
export function ProofSection() {
  return (
    <Section
      ...
      description="Quantified impact from founding engineering, automation consulting, and financial analytics—each card notes scope so the numbers stay grounded."
    >
      <div className="reveal-stagger-grid grid min-w-0 gap-4 sm:gap-5 md:grid-cols-2">
        {metrics.map((metric, index) => {
          ...
          return (
            <article
              key={metric.label}
              className={`relative isolate min-w-0 overflow-x-clip rounded-2xl border border-[var(--border-medium)] p-6 shadow-[inset_0_1px_0_color-mix(in_oklab,var(--foreground)_6%,transparent),var(--shadow-elevated)] ${
            ...
            >
              ...
              <div className="relative z-10">
                ...
                <p
                  className={`mt-3 text-sm leading-relaxed text-[var(--foreground-muted)] ${featured ? "sm:text-base sm:leading-relaxed" : ""}`}
                >
                  {metric.detail}
                </p>
              </div>
              ...
              >
                <p className="font-mono text-[10px] uppercase tracking-[0.16em] text-[var(--muted)] sm:text-[11px] sm:tracking-[0.18em]">
                  Scope & definition
                </p>
                <p className="text-scope mt-2 text-xs leading-relaxed sm:text-sm">{metric.context}</p>
              </div>
            </article>
          );
        })}
