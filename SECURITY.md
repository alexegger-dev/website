...
- This repo is a **static** site: no app server, database, or sessions in-tree. Build-time config is **`NEXT_PUBLIC_*`** URL overrides only ([`.env.example`](.env.example)). Published bundles live under **`docs/`** or CI artifacts—see **README.md** for how that is produced.
